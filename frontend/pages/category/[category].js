...
export async function getServerSideProps({
  ...
  const reqCategory = await fetch(process.env.NEXT_PUBLIC_URLAPI +
                                  "/categories?slug=" + categorySlug);
  const category = await reqCategory.json();
  const reqPosts = await fetch(process.env.NEXT_PUBLIC_URLAPI +
                               "/posts?_where[category.slug]=" + categorySlug);
...
export default function Posts({posts, category}) {
  return (
      <><Head><title>Category & mdash; {category.name}</title>
      </Head>
      <Container><SectionHeader>{category.name}<
          /SectionHeader>
        <PostList
          posts={posts}
